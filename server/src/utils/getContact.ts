...
export async function getContactByPushName(
  pushname: string,
  client: wppconnect.Whatsapp,
): Promise<ContactItem | null> {
  try {
    const contacts = (await client.getAllContacts())
      .filter((c: any) => c.pushname === pushname)
      .map((c: any) => {
        if (c.server === 'c.us') {
          return {
            id: c.id._serialized,
    ...
    const _contact: any = {
      ...
      verifiedName:
        contacts.find((c: any) => c.server === 'c.us')?.verifiedName || null,
      phone_number: contacts.find((c: any) => c.server === 'c.us')
        ?.phone_number,
      phone_formated:
        formatBR(
          contacts.find((c: any) => c.server === 'c.us')?.phone_number,
        ) ||
        contacts.find((c: any) => c.server === 'c.us')?.phone_number ||
        null,
      isBlocked:
        contacts.find((c: any) => c.server === 'c.us')?.isBlocked || false,
      isBusiness:
        contacts.find((c: any) => c.server === 'c.us')?.isBusiness || false,
      type: contacts.find((c: any) => c.server === 'c.us')?.type || null,
      avatar_url:
        contacts.find((c: any) => c.server === 'c.us')?.avatar_url ||
        contacts.find((c: any) => c.server === 'lid')?.avatar_url ||
        contacts.find((c: any) => c.server === 'g.us')?.avatar_url ||
        null,
      server: contacts.find((c: any) => c.server === 'c.us')?.server,
      whatsapp_id:
        contacts.find((c: any) => c.server === 'lid')?.id ||
        contacts.find((c: any) => c.server === 'c.us')?.id ||
        contacts.find((c: any) => c.server === 'g.us')?.id,
    };
