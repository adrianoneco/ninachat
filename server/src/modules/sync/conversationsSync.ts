...
export async function syncConversations(
  ...
  convs.forEach(async (conv: any) => {
    const contact = await contactRepo
      ...
      .orWhere('c.phone_number = :value', {
        value: conv.contact.id._serialized.split('@')[0],
      })
      .getOne();
    ...
    convRepo
      ...
      .then(() => {
        console.log(
          `[${instance.name}] synced conversation: ${conv.id} (${contact ? contact.name : 'unknown contact'})`,
        );
      })
      .catch((e) => {
        console.warn(
          `[${instance.name}] failed syncing conversation ${conv.id}: ${String(e)}`,
        );
      });

    const existing: any = await convRepo.findOneBy({
      chat_id: `${instance.id}-${contact?.whatsapp_id}`,
    });
