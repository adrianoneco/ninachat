...
export async function syncContacts(
  ...
  console.log(
    `[SYNC] [CONTACTS] Processing contacts for instance ${instance.name} (${instance.id})...`,
  );

  (await client.getAllContacts())
    .filter(
      (c: any) =>
        !['status@broadcast', '0@c.us'].includes(c.id?._serialized) &&
        ['c.us', 'lid', 'g.us'].includes(c?.id?.server),
    )
    .forEach((contact: any) => {
  ...
  Object.values(contacts).forEach((group: any) => {
    ...
    group.forEach((c: any) => {
      if (c.server === 'c.us') {
        ...
        parameter['phone_formated'] =
          formatBR(c.phone_number) || c.phone_number;
  ...
  for await (const contact of _contacts_serialized) {
    ...
    await contactRepo
      ...
      .then(() =>
        console.log(
          `Contact ${contact.name} (${contact.phone_number}) synced successfully.`,
        ),
      )
      .catch((err) =>
        console.error(
          `Failed to sync contact ${contact.name} (${contact.phone_number}):`,
          err,
        ),
      );
  }
