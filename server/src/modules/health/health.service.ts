...
export class HealthService {
  ...
  async check() {
    ...
    try {
      ...
      if (data.site_name && String(data.site_name).trim()) {
        results.push({
          component: 'identity',
          status: 'ok',
          message: 'Identidade configurada',
        });
      } else {
        results.push({
          component: 'identity',
          status: 'warning',
          message: 'Nome da empresa não configurado',
        });
      }
    } catch {
      results.push({
    ...
    try {
      ...
      if (cnt > 0) {
        results.push({
          component: 'pipeline',
          status: 'ok',
          message: `Pipeline configurado (${cnt} etapas)`,
        });
      } else {
        results.push({
          component: 'pipeline',
          status: 'warning',
          message: 'Nenhuma etapa de pipeline configurada',
        });
      }
    } catch {
      results.push({
        component: 'pipeline',
        status: 'warning',
        message: 'Não foi possível verificar pipeline',
      });
    }

    // 4. WhatsApp instance
    try {
      const rows = await this.ds.query(`SELECT status FROM instances LIMIT 1`);
      if (rows.length === 0) {
        results.push({
          component: 'whatsapp',
          status: 'warning',
        ...
        if (
          ...
          results.push({
            component: 'whatsapp',
            status: 'ok',
            message: `WhatsApp conectado (${st})`,
          });
        } else {
          results.push({
            component: 'whatsapp',
            status: 'warning',
            message: `WhatsApp desconectado (${st ?? 'desconhecido'})`,
          });
        }
      }
    } catch {
      results.push({
        component: 'whatsapp',
        status: 'warning',
        message: 'Não foi possível verificar WhatsApp',
      });
    }
    ...
    try {
      ...
      if (rows.length === 0) {
        results.push({
          component: 'livechat_settings',
          status: 'warning',
          message: 'Configurações do agente não encontradas',
        });
      } else if (rows[0].is_active) {
        results.push({
          component: 'livechat_settings',
          status: 'ok',
          message: `Agente "${rows[0].agent_name || 'LiveChat'}" ativo`,
        });
      } else {
        results.push({
          component: 'livechat_settings',
          status: 'warning',
          message: 'Agente IA inativo',
        });
      }
    } catch {
      results.push({
        component: 'livechat_settings',
        status: 'warning',
        message: 'Não foi possível verificar agente IA',
      });
    }
    ...
    try {
      ...
      if (cnt > 0) {
        results.push({
          component: 'profile',
          status: 'ok',
          message: `Equipe configurada (${cnt} membros)`,
        });
      } else {
        results.push({
          component: 'profile',
      ...
      results.push({
        component: 'profile',
        status: 'warning',
        message: 'Não foi possível verificar equipe',
      });
    }
