...
const DEFAULT_PERMISSIONS = [
  {
    key: 'instances.manage',
    description: 'Create/update/delete/restart instances',
  },
...
export class RolesService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    const gerente = await this.roleRepo.findOne({
      where: { name: 'Gerente' } as any,
    });
    if (!gerente) {
      const perms = await this.permRepo.find({
        where: { key: In(['reports.view', 'contacts.sync']) },
      });
    ...
    const supervisor = await this.roleRepo.findOne({
      where: { name: 'Supervisor' } as any,
    });
    if (!supervisor) {
      const perms = await this.permRepo.find({
        where: { key: In(['messages.read', 'contacts.sync']) },
      });
      await this.roleRepo.save(
        this.roleRepo.create({
          name: 'Supervisor',
    ...
    const atendente = await this.roleRepo.findOne({
      where: { name: 'Atendente' } as any,
    });
    if (!atendente) {
      const perms = await this.permRepo.find({
        where: { key: In(['messages.read', 'messages.send']) },
      });
      await this.roleRepo.save(
        this.roleRepo.create({
          name: 'Atendente',
  ...
  async ensureAdminNotLost(roleId: string, newPermKeys: string[]) {
    ...
    const admins = await this.userRepo
      ...
      .leftJoin('u.roles', 'r')
      .where('r.name = :name', { name: 'Administrador' })
      .getMany();
    if (admins.length === 0 && (!newPermKeys || newPermKeys.length === 0)) {
      throw new BadRequestException(
        'Cannot remove administrator permissions — no admin would remain',
      );
    }
  }
}
