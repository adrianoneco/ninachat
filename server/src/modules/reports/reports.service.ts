...
export class ReportsService {
  ...
  async getMetrics(days: number) {
    ...
    const [curConvs, prevConvs] = await Promise.all([
      this.convRepo.createQueryBuilder('c').where('c.created_at >= :from', { from }).getCount(),
      this.convRepo.createQueryBuilder('c').where('c.created_at >= :from AND c.created_at < :to', { from: prevFrom, to: from }).getCount(),
    ]);

    // Converted (closed conversations)
    const [curClosed, prevClosed] = await Promise.all([
      this.convRepo.createQueryBuilder('c').where("c.status = 'closed' AND c.created_at >= :from", { from }).getCount(),
      this.convRepo.createQueryBuilder('c').where("c.status = 'closed' AND c.created_at >= :from AND c.created_at < :to", { from: prevFrom, to: from }).getCount(),
    ]);

    // New contacts
    const [curContacts, prevContacts] = await Promise.all([
      this.contactRepo.createQueryBuilder('c').where('c.created_at >= :from', { from }).getCount(),
      this.contactRepo.createQueryBuilder('c').where('c.created_at >= :from AND c.created_at < :to', { from: prevFrom, to: from }).getCount(),
    ]);
    ...
    return [
      { label: 'Atendimentos', value: String(curConvs), trend: pct(curConvs, prevConvs), trendUp: up(curConvs, prevConvs) },
      { label: 'Conversões', value: String(curClosed), trend: pct(curClosed, prevClosed), trendUp: up(curClosed, prevClosed) },
      { label: 'Tempo Médio', value: avgTimeStr, trend: '', trendUp: true },
      { label: 'Novos Leads', value: String(curContacts), trend: pct(curContacts, prevContacts), trendUp: up(curContacts, prevContacts) },
    ];
  }
  ...
  async getChart(days: number) {
    ...
    const convMap = new Map(convRows.map(r => [String(r.day).slice(0, 10), Number(r.cnt)]));
    const msgMap = new Map(msgRows.map(r => [String(r.day).slice(0, 10), Number(r.cnt)]));
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      const name =
        days === 1 ? 'Hoje' :
        days <= 7 ? ['Dom','Seg','Ter','Qua','Qui','Sex','Sáb'][d.getDay()] :
        `${d.getDate()}/${d.getMonth() + 1}`;
      result.push({ name, chats: convMap.get(key) ?? 0, sales: msgMap.get(key) ?? 0 });
    }
    return result;
  }
  ...
  async getFull(days: number, instanceId?: string, agentId?: string) {
    ...
    const hourlyRows: any[] = await this.ds
      ...
      `, [from])
      .catch(() => []);
    const hourlyMap = new Map(hourlyRows.map(r => [Number(r.hour), Number(r.cnt)]));
    ...
    const weekdayRows: any[] = await this.ds
      ...
      `, [from])
      .catch(() => []);
    const weekdayMap = new Map(weekdayRows.map(r => [Number(r.dow), Number(r.cnt)]));
    const weekdayVolume = ['Dom','Seg','Ter','Qua','Qui','Sex','Sáb'].map((day, i) => ({
      day, value: weekdayMap.get(i) ?? 0,
    }));

    // Agent performance from generic_records (team_members) joined with messages
    const teamRows: any[] = await this.ds
      .query(`SELECT data FROM generic_records WHERE collection = 'team_members'`)
      .catch(() => []);
      ...
      teamRows.map(async (row) => {
        ...
        const [total, closed, avgRow] = await Promise.all([
          this.convRepo.createQueryBuilder('c')
            .where('c.assigned_user_id = :uid AND c.created_at >= :from', { uid: userId, from })
            .getCount()
            .catch(() => 0),
          this.convRepo.createQueryBuilder('c')
            .where("c.assigned_user_id = :uid AND c.status = 'closed' AND c.created_at >= :from", { uid: userId, from })
            .getCount()
            .catch(() => 0),
          this.ds
            .query(`
        ...
        return {
          ...
          total,
          closed,
          avgResponseMinutes: Math.round(parseFloat(avgRow[0]?.avg_min ?? '0') || 0),
        };
      }),
    );

    return { statusDistribution, mediaDistribution, hourlyActivity, weekdayVolume, agentPerformance };
  }
}
