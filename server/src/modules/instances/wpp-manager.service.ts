...
export class WppManagerService {
  ...
  async startInstance(instance: Instance) {
    ...
    const contactsPath = `${process.env.DATA_DIR}/${instance.id}/contacts`;
    const conversationsPath = `${process.env.DATA_DIR}/${instance.id}/conversations`;


    if (!fs.existsSync(conversationsPath)) {
      fs.mkdirSync(conversationsPath, { recursive: true });

      (await client.listChats()).forEach(async (conv: any) => {
        const lid = conv.accountLid?._serialized;

        fs.writeFileSync(`${conversationsPath}/${lid}.json`, JSON.stringify(conv, null, 4));
        console.log(`Processed conversation ${conv.id} for contact ${lid}`);


        if (lid !== undefined) {
          await this.convRepo.query(`INSERT INTO "conversations"("lid", "instance_id") VALUES('${lid}', '${instance.id}') ON CONFLICT DO NOTHING;`);
        }
      });
    }
