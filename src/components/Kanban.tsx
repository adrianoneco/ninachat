...
// Mock mode - no supabase import needed
...
const Kanban: React.FC = () => {
  ...
  return (
    <div className="h-full flex flex-col bg-white dark:bg-slate-950 text-gray-900 dark:text-slate-50 p-6 overflow-hidden relative">
      {/* Header */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4 flex-shrink-0">
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Pipeline de Vendas</h2>
          <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">Gerencie oportunidades e acompanhe o fluxo de receita.</p>
        </div>
        <div className="flex gap-3 w-full sm:w-auto">
          <div className="relative flex-1 sm:w-64">
             ...
             <input 
                ...
                className="w-full pl-9 pr-4 py-2 bg-gray-100 dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-lg text-sm text-gray-700 dark:text-slate-200 focus:ring-1 focus:ring-ring/50 outline-none placeholder:text-gray-400 dark:text-slate-600"
             />
          </div>
          <Button 
            variant="outline" 
            className="border-gray-300 dark:border-slate-700 text-gray-600 dark:text-slate-300 hover:bg-gray-200 dark:bg-slate-800"
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-2 custom-scrollbar">
                  {columnDeals.map((deal) => (
                    <div
                      ...
                      className="bg-gray-100 dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-lg p-3 shadow-sm cursor-grab active:cursor-grabbing hover:border-border/50 transition-all group relative"
                    >
                      <div className="flex justify-between items-start mb-1.5">
                        ...
                        <button className="text-gray-400 dark:text-slate-600 hover:text-gray-900 dark:text-white transition-colors opacity-0 group-hover:opacity-100">
                           <MoreHorizontal className="w-3.5 h-3.5" />
                        </button>
                      </div>

                      <h4 className="font-semibold text-gray-900 dark:text-white text-sm mb-0.5 leading-tight">{deal.title}</h4>
                      <p className="text-[10px] text-gray-500 dark:text-slate-400 mb-2">{deal.company}</p>

                      <div className="flex items-center gap-1.5 mb-2 flex-wrap">
                         {deal.tags.map(tag => (
                             <span key={tag} className="text-[9px] text-gray-500 dark:text-slate-500 bg-gray-200 dark:bg-slate-800 px-1.5 py-0.5 rounded flex items-center gap-1">
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-gray-200 dark:border-slate-800">
                         <div className="flex items-center gap-1.5 text-gray-600 dark:text-slate-300 text-xs font-bold">
                            <DollarSign className="w-3 h-3 text-emerald-500" />
                            {formatCurrency(deal.value)}
                         </div>
                         <div className="flex items-center gap-2">
                            {deal.dueDate && (
                                <div className="text-[9px] text-gray-500 dark:text-slate-500 flex items-center gap-1" title="Data de previsão">
                                    <CalendarClock className="w-3 h-3" />
                                    {new Date(deal.dueDate).toLocaleDateString('pt-BR', {day: '2-digit', month: '2-digit'})}
                                </div>
                            )}
                            <img src={deal.ownerAvatar} alt="Owner" className="w-5 h-5 rounded-full border border-gray-300 dark:border-slate-700" />
                         </div>
                      </div>
                    </div>
                  ))}
      ...
      {/* Drawer */}
      <div 
        className={`fixed top-0 right-0 h-full w-full max-w-2xl bg-white dark:bg-slate-950 border-l border-gray-200 dark:border-slate-800 shadow-2xl z-50 transform transition-transform duration-300 ease-in-out flex flex-col ${selectedDeal ? 'translate-x-0' : 'translate-x-full'}`}
      >
        {selectedDeal && (
            <>
                {/* 1. Header & Stage Progress */}
                <div className="flex-shrink-0 bg-gray-100 dark:bg-slate-900 border-b border-gray-200 dark:border-slate-800">
                    {/* Top Bar */}
                    <div className="p-6 pb-4 flex justify-between items-start">
                        <div>
                            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-1">{selectedDeal.title}</h2>
                            <div className="flex items-center gap-2 text-gray-500 dark:text-slate-400 text-sm flex-wrap">
                                ...
                                <Select value={selectedDeal.ownerId || ''} onValueChange={handleOwnerChange}>
                                  <SelectTrigger className="w-[180px] h-7 text-xs bg-gray-200 dark:bg-slate-800 border-gray-300 dark:border-slate-700">
                        ...
                        <div className="flex gap-2">
                            ...
                            <button 
                                onClick={() => setSelectedDeal(null)} 
                                className="p-2 hover:bg-gray-200 dark:bg-slate-800 rounded-lg text-gray-500 dark:text-slate-400 hover:text-gray-900 dark:text-white transition-colors"
                            >
                                <X className="w-6 h-6" />
                            </button>
                        </div>
                            ...
                            {stages.map((col, idx) => {
                                ...
                                return (
                                    <div 
                                        ...
                                        className={`flex-1 h-8 flex items-center justify-center px-2 relative cursor-pointer group transition-all first:rounded-l-md last:rounded-r-md 
                                            ${isCompleted ? 'bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/30' : 
                                              isActive ? 'bg-primary text-white dark:text-white shadow-lg shadow-[0_10px_30px_rgba(30,95,116,0.16)]' : 
                                              'bg-gray-200 dark:bg-slate-800 text-gray-500 dark:text-slate-500 hover:bg-gray-300 dark:bg-slate-700 hover:text-gray-600 dark:text-slate-300'}
                                        `}
                                        onClick={async () => {
                                    ...
                                    >
                                        ...
                                        {idx !== stages.length - 1 && (
                                            <div className="absolute right-0 top-0 bottom-0 w-[1px] bg-gray-50/20 dark:bg-slate-950/20 z-20"></div>
                                        )}
                                    </div>
                                )
                            })}
                ...
                <div className="flex-1 overflow-y-auto bg-white dark:bg-slate-950 custom-scrollbar">
                    ...
                    {/* Action Composer */}
                    <div className="p-6 border-b border-gray-200 dark:border-slate-800 bg-gray-100/30 dark:bg-slate-900/30">
                        <div className="flex gap-4 mb-4">
                            <button 
                                onClick={() => setActiveTab('note')}
                                className={`flex items-center gap-2 text-sm font-medium transition-colors ${activeTab === 'note' ? 'text-cyan-400' : 'text-gray-500 dark:text-slate-400 hover:text-gray-900 dark:text-white'}`}
                            >
                                <div className={`p-2 rounded-full ${activeTab === 'note' ? 'bg-cyan-500/10' : 'bg-gray-200 dark:bg-slate-800'}`}>
                            ...
                            <button 
                                ...
                                className={`flex items-center gap-2 text-sm font-medium transition-colors ${activeTab === 'activity' ? 'text-amber-400' : 'text-gray-500 dark:text-slate-400 hover:text-gray-900 dark:text-white'}`}
                            >
                                <div className={`p-2 rounded-full ${activeTab === 'activity' ? 'bg-amber-500/10' : 'bg-gray-200 dark:bg-slate-800'}`}>
                            ...
                            <button 
                                ...
                                className={`flex items-center gap-2 text-sm font-medium transition-colors ${activeTab === 'email' ? 'text-violet-400' : 'text-gray-500 dark:text-slate-400 hover:text-gray-900 dark:text-white'}`}
                            >
                                <div className={`p-2 rounded-full ${activeTab === 'email' ? 'bg-violet-500/10' : 'bg-gray-200 dark:bg-slate-800'}`}>
                        ...
                        <div className="bg-gray-100 dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl overflow-hidden focus-within:ring-1 focus-within:ring-cyan-500/50 transition-all shadow-inner">
                            <input 
                                type="text"
                                className="w-full bg-transparent p-3 text-sm text-gray-700 dark:text-slate-200 placeholder:text-gray-400 dark:text-slate-600 outline-none border-b border-gray-200 dark:border-slate-800"
                                placeholder="Título da atividade"
                                value={newActivityTitle}
                                onChange={(e) => setNewActivityTitle(e.target.value)}
                            />
                            <textarea 
                                className="w-full bg-transparent p-4 text-sm text-gray-700 dark:text-slate-200 placeholder:text-gray-400 dark:text-slate-600 outline-none resize-none min-h-[80px]"
                                placeholder={
                            ...
                            />
                            <div className="px-3 py-2 bg-white/50 dark:bg-slate-950/50 border-t border-gray-200 dark:border-slate-800 flex justify-between items-center">
                                <div className="flex gap-2">
                                    <button className="p-1.5 hover:bg-gray-200 dark:bg-slate-800 rounded text-gray-500 dark:text-slate-400 hover:text-cyan-400 transition-colors"><Paperclip className="w-4 h-4" /></button>
                                </div>
                    ...
                    {/* Activities Timeline */}
                    <div className="p-6">
                        <h4 className="text-xs font-bold text-gray-500 dark:text-slate-500 uppercase tracking-wider mb-6 flex items-center gap-2">
                          ...
                          <div className="space-y-3">
                            {activities.map(activity => {
                              ...
                              const activityColor = activity.type === 'call' ? 'text-amber-500 bg-amber-500/10' :
                                                    ...
                                                    'text-gray-500 dark:text-slate-500 bg-slate-500/10';
                              const ActivityIcon = activityIcon;
                              
                              return (
                                <div key={activity.id} className="flex items-start gap-3 p-3 rounded-xl bg-gray-100/50 dark:bg-slate-900/50 border border-gray-200 dark:border-slate-800 hover:border-gray-300 dark:border-slate-700 transition-all group">
                                  <button 
                                    onClick={() => handleToggleActivityComplete(activity.id, activity.isCompleted)}
                                    className="mt-0.5 text-gray-500 dark:text-slate-500 hover:text-emerald-500 transition-colors"
                                  >
                                  ...
                                  <div className="flex-1">
                                    <p className={`text-sm font-medium transition-colors ${activity.isCompleted ? 'text-gray-500 dark:text-slate-500 line-through' : 'text-gray-700 dark:text-slate-200 group-hover:text-gray-900 dark:text-white'}`}>
                                      {activity.title}
                                    </p>
                                    {activity.description && (
                                      <p className="text-xs text-gray-500 dark:text-slate-500 mt-1">{activity.description}</p>
                                    )}
                                  ...
                                  <button 
                                    ...
                                    className="opacity-0 group-hover:opacity-100 p-1 hover:bg-red-500/10 rounded text-gray-500 dark:text-slate-500 hover:text-red-500 transition-all"
                                  >
                    ...
                    {selectedDeal.clientMemory && (
                      <div className="p-6 border-t border-gray-200 dark:border-slate-800">
                        ...
                        <div className="space-y-3">
                          {/* Qualification Score */}
                          <div className="p-3 rounded-lg bg-gray-100/50 dark:bg-slate-900/50 border border-gray-200 dark:border-slate-800">
                            <div className="flex items-center justify-between mb-2">
                              <span className="text-xs text-gray-500 dark:text-slate-400">Score de Qualificação</span>
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <div className="p-3 rounded-lg bg-gray-100/50 dark:bg-slate-900/50 border border-gray-200 dark:border-slate-800">
                              <span className="text-xs text-gray-500 dark:text-slate-400">Orçamento</span>
                              <p className="text-sm text-gray-700 dark:text-slate-200 mt-1 font-medium">
