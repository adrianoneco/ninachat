...
export const SystemHealthCard: React.FC = () => {
  ...
  const getOverallGradient = (status?: 'ok' | 'warning' | 'error') => {
    switch (status) {
      ...
      default:
        return 'from-gray-400/20 to-gray-400/5 border-gray-400/30 dark:from-slate-500/20 dark:to-slate-500/5 dark:border-slate-500/30';
    }
  };

  if (loading) {
    return (
      <div className={`rounded-2xl border bg-gradient-to-br from-gray-200/50 to-gray-300/50 dark:from-slate-800/50 dark:to-slate-900/50 border-gray-300/50 dark:border-slate-700/50 p-6`}>
        <div className="flex items-center justify-center gap-3">
          <Loader2 className="w-5 h-5 animate-spin text-cyan-400" />
          <span className="text-sm text-gray-500 dark:text-slate-400">Verificando sistema...</span>
        </div>
      </div>
    );
  }
    ...
    >
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900 dark:text-white">Status do Sistema</h3>
            <p className="text-xs text-gray-500 dark:text-slate-400">{healthData.message}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={fetchHealth}
            disabled={loading}
            className="p-2 rounded-lg hover:bg-gray-300/50 dark:hover:bg-slate-700/50 transition-colors text-gray-500 dark:text-slate-400 hover:text-gray-900 dark:text-white"
          >
      ...
      {/* Progress Bar */}
      <div className="h-2 bg-gray-200 dark:bg-slate-800 rounded-full overflow-hidden mb-4">
        <motion.div
          initial={{ width: 0 }}
      ...
      <button
        ...
        className="w-full flex items-center justify-between py-2 text-sm text-gray-500 dark:text-slate-400 hover:text-gray-900 dark:text-white transition-colors"
      >
