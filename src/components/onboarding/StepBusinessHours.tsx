  ...
  return (
    ...
    >
      <motion.div variants={itemVariants} className="text-center mb-8">
        ...
        <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">Horário Comercial</h3>
      ...
      <div className="space-y-6 max-w-md mx-auto">
        ...
        <motion.div variants={itemVariants} className="space-y-2">
          <Label className="text-gray-600 dark:text-slate-300 flex items-center gap-2">
            <Globe className="w-4 h-4 text-gray-500 dark:text-slate-500" />
            Fuso Horário
          </Label>
          <Select value={timezone} onValueChange={onTimezoneChange}>
            ...
            <SelectContent className="bg-gray-200 dark:bg-slate-800 border-gray-300 dark:border-slate-700 z-50">
              {TIMEZONES.map((tz) => (
                <SelectItem key={tz.id} value={tz.id} className="text-gray-900 dark:text-white hover:bg-violet-500/20 focus:bg-violet-500/20 focus:text-gray-900 dark:text-white">
                  {tz.name}
                </SelectItem>
              ))}
            </SelectContent>
        ...
        <motion.div variants={itemVariants} className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <span className="text-xs text-gray-500 dark:text-slate-500">Início</span>
              <Select value={businessHoursStart} onValueChange={onBusinessHoursStartChange}>
                <SelectTrigger className="bg-gray-200/50 dark:bg-slate-800/50 border-gray-300 dark:border-slate-700 text-gray-900 dark:text-white">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="bg-gray-200 dark:bg-slate-800 border-gray-300 dark:border-slate-700 max-h-60 z-50">
                  {HOURS.map((hour) => (
                    <SelectItem key={hour.id} value={hour.id} className="text-gray-900 dark:text-white hover:bg-violet-500/20 focus:bg-violet-500/20 focus:text-gray-900 dark:text-white">
                      {hour.name}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div className="space-y-1">
              <span className="text-xs text-gray-500 dark:text-slate-500">Fim</span>
              ...
              <motion.button
                ...
                className={`
                  ...
                  ${businessDays.includes(day.id)
                    ? 'bg-orange-500 text-gray-900 dark:text-white shadow-lg shadow-orange-500/30'
                    : 'bg-gray-200/50 dark:bg-slate-800/50 text-gray-500 dark:text-slate-400 border border-gray-300 dark:border-slate-700 hover:border-orange-500/50'
                  }
                `}
        ...
        <motion.div 
          ...
          className="mt-8 p-4 rounded-xl bg-gray-200/30 dark:bg-slate-800/30 border border-gray-300/50 dark:border-slate-700/50"
        >
          <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-slate-500 mb-3">
            <Eye className="w-3 h-3" />
            Preview do Horário
          </div>
          <div className="space-y-2 text-sm">
            <p className="text-gray-600 dark:text-slate-300">
              <span className="text-gray-500 dark:text-slate-500">Fuso:</span>{' '}
              <span className="text-gray-900 dark:text-white font-medium">{selectedTimezone?.short || timezone}</span>
            </p>
            <p className="text-gray-600 dark:text-slate-300">
              <span className="text-gray-500 dark:text-slate-500">Horário:</span>{' '}
              <span className="text-orange-400 font-medium">{businessHoursStart} - {businessHoursEnd}</span>
            </p>
            <p className="text-gray-600 dark:text-slate-300">
              <span className="text-gray-500 dark:text-slate-500">Dias:</span>{' '}
              <span className="text-gray-900 dark:text-white font-medium">{formattedDays || 'Nenhum selecionado'}</span>
            </p>
          </div>
        </motion.div>
      </div>
